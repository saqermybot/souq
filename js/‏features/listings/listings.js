...
import { fetchListingsPage, fetchListingById, softDeleteListing } from "../../services/listingsRepo.js";
...
export function initListings(){
  ...
  // ✅ حذف الإعلان (لصاحب الإعلان فقط)
  if (UI.el.btnDelete) {
    UI.el.btnDelete.onclick = async () => {
      const l = UI.state.currentListing;
      if (!l) return;
      const ok = confirm("تأكيد حذف الإعلان؟ (سيختفي من القائمة)");
      if (!ok) return;
      try {
        await softDeleteListing(l.id);
...
function syncOwnerActions(listing){
  const me = auth.currentUser?.uid || "";
  const ownerId = listing?.ownerId || "";
  const isOwner = !!me && !!ownerId && me === ownerId;

  if (UI.el.btnDelete) {
    UI.el.btnDelete.classList.toggle("hidden", !isOwner);
  }
  if (UI.el.btnChat) {
    UI.el.btnChat.textContent = isOwner ? "الرسائل" : "مراسلة";
  }
}
...
async function openDetails(id, data = null, fromHash = false){
  try{
    if (!data){
      const got = await fetchListingById(id);
      if (!got) return alert("الإعلان غير موجود أو تم حذفه.");
      data = got;
      id = got.id;
    }

    UI.state.currentListing = { id, ...data };
    syncOwnerActions(UI.state.currentListing);
...
async function loadListings(reset = true){
  ...
  const { docs, lastDoc } = await fetchListingsPage({
    lastDoc: UI.state.lastDoc || null,
    pageSize: 12
  });

  if (docs.length){
    UI.state.lastDoc = lastDoc;
  }else{
